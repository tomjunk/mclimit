...
//    24 Jun 2010 Revert to quadratic asymmetric errors with truncation, put in Wade's priors from 7 Dec. as an option
//     8 Jul 2010 make the auto-prescan in bayeslimit_mcmc1 work better with short truncated intervals for nuisance parameters
//     4 Apr 2011 Add in MCMC cross section fitters.  1D, 2D, and with pseudoexperiments to get expected distributions.
//    12 Apr 2011 Make CLs+b rate limit calculators
//    12 Apr 2011 Make data-fitted bg-subtracted plot information.
//    16 Apr 2011 Add an option not to include bin by bin uncertainty posterior histos in the output rootfile of mcmc1
//                and make it the default.
//     2 Jun 2011 Add an "Asimov" limit calculator to the mcmc1 routines
//    16 Jun 2011 Add a calculator for the -2lnL(avg over nuisance parameters)
//    29 Jun 2011 Bugfix to the mcmc1 likelihood -- in the 2D case without constraining data to be integer had a typo
//     7 Jul 2011 Bugfix to csm_channel_model::interpolate() and bulletproof interpolators
//    14 Jul 2011 Pass through the dumpsfp argument in bayeslimit_mcmc1 as an optional argument
...
#define MCLIMIT_CSM_VERSION_NUMBER 4.44
#define MCLIMIT_CSM_VERSION_DATE "May 29, 2011"
...
mclimit_csm::mclimit_csm()
{
  ...
  // default parameter values for the mcmc1 algorithm are in the header file
  // as default arguments

  this->set_mcmc1_burnin_length();
  this->set_mcmc1_accept_interval();
  this->set_mcmc1_accept_low();
  this->set_mcmc1_accept_high();
  this->set_mcmc1_accept_dimscale();

  // default not to include potentially tens of thousands of histograms of Poisson
  // posteriors of each bin of each template in the output rootfile

  this->disable_poisson_posteriorsinrootfile_mcmc1();
}
...
void mclimit_csm::set_datahist(TH1 *h, char *cname)
{
  ...
  if (ifound == -1)
	...
	{
          dhname.push_back(s);
          datahist.push_back(copy_TH1<TH1> (*h));
	}
      else
	{
	  ...
	  datahist.insert(dhi, copy_TH1<TH1> (*h));
	}
    }
  else
    {
      delete datahist[ifound];
      datahist[ifound] = copy_TH1<TH1> (*h);
    }
}
...
Double_t mclimit_csm::s95sb()
{
  return(s95aux(MCLIMIT_CSM_CLSB));
}

/*----------------------------------------------------------------------------*/

Double_t mclimit_csm::s95m2sb()
{
  return(s95aux(MCLIMIT_CSM_CLSBM2));
}

/*----------------------------------------------------------------------------*/

Double_t mclimit_csm::s95m1sb()
{
  return(s95aux(MCLIMIT_CSM_CLSBM1));
}

/*----------------------------------------------------------------------------*/

Double_t mclimit_csm::s95medsb()
{
  return(s95aux(MCLIMIT_CSM_CLSBMED));
}

/*----------------------------------------------------------------------------*/

Double_t mclimit_csm::s95p1sb()
{
  return(s95aux(MCLIMIT_CSM_CLSBP1));
}

/*----------------------------------------------------------------------------*/

Double_t mclimit_csm::s95p2sb()
{
  return(s95aux(MCLIMIT_CSM_CLSBP2));
}
...
Double_t mclimit_csm::s95aux(Int_t itype)
{
  ...
  for (j=0;j<32;j++)
      ...
      if (foundit == 0)
	{
	  ...
	  else if (itype == MCLIMIT_CSM_CLSBP1) cltest = clsbexpbp1();
	  else if (itype == MCLIMIT_CSM_CLSBP2) cltest = clsbexpbp2();
	  else
	    {
	      std::cout << "Ununderstood option in s95aux: " << itype << std::endl;
	      exit(0);
	    }
	  delete scaledsignal;
      ...
      for( int j=-5; j<6; j+=2 )
	{
	  ...
	  csm_model* scaledsignal = testhypsave->scalesignal(sf);
          ...
          else if (itype == MCLIMIT_CSM_CLSP2)
	    {
          ...
          else if (itype == MCLIMIT_CSM_CLSB)
	    {
	      cltest = clsb();
	      clbtest = 1;
	      clsbtest = clsb();
  	    }
          else if (itype == MCLIMIT_CSM_CLSBM2)
	    {
	      cltest = clsbexpbm2();
	      clbtest = 1;
	      clsbtest = clsbexpbm2();
	    }
          else if (itype == MCLIMIT_CSM_CLSBM1)
	    {
	      cltest = clsbexpbm1();
	      clbtest = 1;
	      clsbtest = clsbexpbm1();
	    }
          else if (itype == MCLIMIT_CSM_CLSBMED)
	    {
	      cltest = clsbexpbmed();
	      clbtest = 1;
	      clsbtest = clsbexpbmed();
	    }
          else if (itype == MCLIMIT_CSM_CLSBP1)
	    {
	      cltest = clsbexpbp1();
	      clbtest = 1;
	      clsbtest = clsbexpbp1();
	    }
          else if (itype == MCLIMIT_CSM_CLSBP2)
	    {
	      cltest = clsbexpbp2();
	      clbtest = 1;
	      clsbtest = clsbexpbp2();
  ...
  for (i=0;i<(Int_t) null_hypothesis_pe->channame.size(); i++)
    {
      ...
      pdarray[i] = copy_TH1<TH1> (*null_hypothesis_pe->chanmodel[i]->histotemplate[0], pdname);
      delete[] pdname;
    }
                                          ...
                                          Double_t paramvalue[])
{
  ...
  */
      ...
      if (vpi != semap.end())
          ...
          for (int k1=0;k1<nsloc;k1++)
	    {
	      ...
	      itpl = syserr[i].itemplate;
              ...
              if (chanpriorfunc == CSM_PRIORFUNC_EXPONENTIAL)
		{
		  ...
		  // the additive version as Wade does it -- see additive code below
		  rsum[itpl] += lnB - 1.0;
		  ...
		  if (syserr[i].highshape != 0)
		    {
		      if (hcl.get() == 0)
			{
			  hcl.reset (copy_TH1<TH1Type> (*histotemplate[itpl]));
			}
		      else
			{
  ...
  // Wade likes summed contributions
  if (chanpriorfunc == CSM_PRIORFUNC_EXPONENTIAL)
    {
...
void csm_model::list_nparams(vector<char *> *npn, vector<Double_t> *nplb, vector<Double_t> *nphb)
{
  ...
  for (i=0;i<(Int_t) channame.size();i++)
      ...
      for (j=0;j<(Int_t) cm->syserr.size();j++)
	{
	  ...
	  // Put them back as an option

	  if (modelpriorfunc == CSM_PRIORFUNC_QUAD)
	    {
...
csm_model::csm_model()
{
  modelpriorfunc = CSM_PRIORFUNC_QUAD;
}
...
void csm_model::setpriorfunc(CSM_PRIORFUNC inputpriorfunc)
{
  modelpriorfunc = inputpriorfunc;
  int nchans = (Int_t) channame.size();
  for (int ichan=0; ichan < nchans; ichan++) chanmodel[ichan]->setpriorfunc(inputpriorfunc);
}

void csm_channel_model::setpriorfunc(CSM_PRIORFUNC inputpriorfunc)
{
  chanpriorfunc = inputpriorfunc;
}
...
csm_model* csm_model::Clone()
{
  ...
  mclone->setpriorfunc(modelpriorfunc);
...
csm_model* csm_model::interpolate(csm_model *b, double frac)
{
  ...
  imodel->setpriorfunc(modelpriorfunc);
...
csm_model* csm_model::scale(Double_t coefficient)
{
  ...
  smodel->setpriorfunc(modelpriorfunc);
  ...
  if (ifound == -1)
    {
      ...
      cm->setpriorfunc(modelpriorfunc);
      strcpy(s,cname);
      if (jfound == -1)
	{
...
csm_channel_model::csm_channel_model()
{
  ...
  chanpriorfunc = CSM_PRIORFUNC_QUAD;   // default prior function
}
 ...
 Int_t sflag)        // scale flag -- 1 if signal, 0 if background (for use with s95 calculator)
{
  ...
  histotemplate.push_back(template_hist_arg = template_hist.copy_TH1<TH1Type>());
  histotemplate_varied.push_back(template_hist.copy_TH1<TH1Type>());
  for (i=0;i<nnp;i++)
    {
      ...
      if (lowshape.has(i))
	{
	  ses.lowshape = lowshape.copy_TH1<TH1Type> (i);
	}
      else
	{
	  ses.lowshape = 0;
      ...
      if (highshape.has(i))
	{
	  ses.highshape = highshape.copy_TH1<TH1Type> (i);
	}
      else
	{
	  ses.highshape = 0;
...
csm_channel_model* csm_channel_model::Clone()
{
  ...
  mclone->setpriorfunc(chanpriorfunc);
...
csm_channel_model* csm_channel_model::interpolate(csm_channel_model *b, double frac)
{
  ...
  mint->setpriorfunc(chanpriorfunc);
      ...
      for (j=0;j < nsys;j++)
	{
	  if (syserr[j].itemplate == i)
	      ...
	      if (syserr[j].lowshape != 0 && b->syserr[j].lowshape != 0)
		{
		  ...
		  htj.reset (copy_TH1<TH1Type> (*syserr[j].lowshape));
		...
		{
		  std::auto_ptr<TH1Type> htj;
		  htj.reset (copy_TH1<TH1Type> (*syserr[j].highshape));
		  if (poissflag[i] == CSM_GAUSSIAN_BINERR)
		    {
      ...
      hti.reset (copy_TH1<TH1Type> (*histotemplate[i]));
      if (poissflag[i] == CSM_GAUSSIAN_BINERR)
	{
...
csm_channel_model* csm_channel_model::scale(Double_t coefficient)
{
  ...
  smodel->setpriorfunc(chanpriorfunc);
...
void csm_channel_model::plotwithdata(TH1* dh)
{
  ...
  datamax = dh->GetBinContent(dh->GetMaximumBin());
  nbinsy = dh->GetNbinsY();
  datamax += sqrt(datamax);
  plotmax = max(datamax,stackmax);
  //cout << "stackmax, datamax, plotmax: " << stackmax << " " << datamax << " " << plotmax << endl;
  hs->SetMaximum(plotmax);
					  ...
					  double *sp1, double *sp2)
{
  ...
  *sm1 = clist[i];
  i =  max(0, min(npx-1, (int) nearbyint(npx*MCLIMIT_CSM_MCLMED)));
  *smed = clist[i];
  i =  max(0, min(npx-1, (int) nearbyint(npx*MCLIMIT_CSM_MCLP1S)));
  *sp1 = clist[i];
  i =  max(0, min(npx-1, (int) nearbyint(npx*MCLIMIT_CSM_MCLP2S)));
  *sp2 = clist[i];

  for (int i=0;i<nchans; i++) delete pdarray[i];
  delete[] pdarray;
  for (int i=0;i<nchans; i++) datahist[i] = dhsavearray[i];
  delete[] dhsavearray;
}
...
// testhyp_pe -- the predictions used to compute the limits.

double mclimit_csm::bayeslimit_mcmc1_expect_asimov(double beta, PRIOR prior, TString histoutfile)
{
  int nchans = null_hypothesis_pe->channame.size();
  TH1** pdarray = new TH1*[nchans];     // data and pseudodata stay as TH1's
  char *pdname;
  ...
  // Add up the null_hypothesis_pe template predictions to make pseudodata
  ...
  double slimit = bayeslimit_mcmc1(beta, prior, histoutfile, false);
...
//-------------------------------------------------------------------------
//  Adaptive Markov Chain tools.  1D limits and cross sections, 2D cross sections
...
// maintenance in case we change the Markov Chain algorithm.

// all arguments are optional -- see the header file for default values

double mclimit_csm::bayeslimit_mcmc1(double beta, PRIOR prior, TString histoutfile, bool forcedatatobeinteger,
                                     bool dumpsfp)
{
  double sflimit;
...
{
  double avg2lnL;
  mcmc1(llfavg,0.95,prior,histoutfile,&avg2lnL);
  return(avg2lnL);
}

// generate a nullhyp_pe pseudoexperiment -- fixing the random seed, and compute the avgmcmc1 with it.
//  This is meant to be used in a loop over the testhyp_pe model, for doing look-elsewhere studies.
// warning -- this has the impact of resetting gRandom's seed to a function of ipx.

double mclimit_csm::minustwolnLavg_mcmc1_pseudoexperiment(int ipx, TString histoutfile, PRIOR prior)
{
  double px2lnLavg=0;
  int nchans = null_hypothesis_pe->channame.size();
  TH1** pdarray = new TH1*[nchans];     // data and pseudodata stay as TH1's
  ...
  if (ipx == -8941) ipx = 771654;  // zero is a special seed; avoid it.
  gRandom->SetSeed(ipx+8941);

   null_hypothesis_pe->single_pseudoexperiment(pdarray);
   px2lnLavg = minustwolnLavg_mcmc1(histoutfile,prior);
...
// generate a nullhyp_pe pseudoexperiment -- fixing the random seed, and compute the chisquared using testhyp_pe with it.
//  This is meant to be used in a loop over the testhyp_pe model, for doing look-elsewhere studies.
// warning -- this has the impact of resetting gRandom's seed to a function of ipx.

double mclimit_csm::nullpe_testpe_pseudoexperiment(int ipx)
{
  double c2val=0;
  int nchans = null_hypothesis_pe->channame.size();
  TH1** pdarray = new TH1*[nchans];     // data and pseudodata stay as TH1's
  ...
  for (int i=0;i<nchans; i++)
   ...
   c2val = calc_chi2(test_hypothesis_pe,pdarray);
                                     ...
                                     Double_t *m1s, Double_t *med, Double_t *p1s, Double_t *p2s, TH1F *xs1dposterior)
{
  ...
  *xsfitavg = 0;
  ...
  // fit the cross section for each pseudoexperiment

  for (int ipx=0; ipx<npx; ipx++)
    {
      double xsfit,downerr,uperr;
      test_hypothesis->single_pseudoexperiment(pdarray);
      xsfit_mcmc1(&xsfit,&downerr,&uperr,xs1dposterior);
      if (pxprintflag)
	{
...
// run npx pseudoexperiments and fit the cross section in 2D for each one. -- Use test_hypothesis
// to generate the pseudoexperiments.

void mclimit_csm::xsfit_2d_mcmc1_expect(int npx, TH2F *xs2dposterior)
{
  int i,ipx,nchans;
  ...
  for (ipx=0;ipx<npx;ipx++)
    {
      test_hypothesis->single_pseudoexperiment(pdarray);
      double sf1fit,sf2fit;
      xsfit_2d_mcmc1(&sf1fit,&sf2fit,xs2dposterior);
...
// histograms are supplied by the user and are filled with log10(s/b) contents for
// signal, background fit to the data, and the data.  The fits are done with the Markov
// Chain routine below with the full separation by channel and bin.  The RMS errors in the
...
// that the binning is uniform (all bins have the same width).

void mclimit_csm::bgfitplot_mcmc1(TH1F *bgfit, TH1F *sighist, TH1F *datahist)
{
  bgfit->Reset();
  sighist->Reset();
  datahist->Reset();
  int nbinsx = bgfit->GetNbinsX();
  double xlow = bgfit->GetXaxis()->GetXmin();
  double xhigh = bgfit->GetXaxis()->GetXmax();
  TProfile bgfitprofile("mcmc1bgfitprofile","mcmc1bgfitprofile",nbinsx,xlow,xhigh);
  mcmc1(bgfitplot,0.95,corr,"",0,0,0,0,false,0,0,0,&bgfitprofile,sighist,datahist);

  //  The TProfile used in mcmc1 contains as errors the RMS divided by the square root of the contents
  //  so remultiply to get the RMS.

  for (int i=1;i<=nbinsx;i++)
    {
      bgfit->SetBinContent(i,bgfitprofile.GetBinContent(i));
      bgfit->SetBinError(i,bgfitprofile.GetBinError(i)*sqrt(bgfitprofile.GetBinEntries(i)));
    }
}

//-------------------------------------------------------------------------------------------------------
// general routine to do Markov Chain calculations -- uses test_hypothesis_pe for all model testing.
// Calculates limits and 1D and 2D cross section fits

void mclimit_csm::mcmc1(MCMC1ACTION action, double beta, PRIOR prior, TString histoutfile,
       		        double *sflimit,
                        double *xsfit, double *downerr, double *uperr, bool dumpsfp,
		        double *xsfit2,TH2F *xs2dposterior, TH1F *xs1dposterior,
			TProfile *bgfit, TH1F *shp, TH1F *dhp, bool forcedatatobeinteger)
{
  csm_channel_model* cm;
  ...
  double npf1[nnptot];
  bool npunconstrained[nnptot];
  bool npconsflag[nnptot];      // if this nuisance parameter is computed as a function of other nuisance parameters
  ...
  vector<double> ssf2list;
  TH1D *totcount = 0;
  double stotdim = sqrt(nnptot + 1.0)/2.0;
  double stotdimp = sqrt(nperrtot + 1.0)/2.0;
  double stotdims = stotdim/2.0;       // see code below -- this gets overwritten.
  double stotdims2 = stotdims;         // for a possible second signal in 2D cross section fits

  const int accept_interval = nmcmc1_accept_interval;    // every so many samples re-evalulate the acceptance rate.
  const double accept_low = rmcmc1_accept_low;     // desire an acceptance ratio between low and high values.
  const double accept_high = rmcmc1_accept_high;
  const double dimscale = rmcmc1_accept_dimscale;  // scale the step by this up or down if we don't have the right acceptance rate.

  vector<int> binindex;                // for bgfitplot use -- keep track of which bin of the target log10(s/b) histogram
  // an input bin goes into
  int nbinslnsb=1;
  if (action==bgfitplot) nbinslnsb = dhp->GetNbinsX()+1;
  double bgfa[nbinslnsb];              // for accumulating a background prediction in the MCMC step
  ...
  // assign log10 s/b bins based on unfluctuated s/b values if we are making the background fit plot
  // fill in the summed signal and data predictions

  if (action == bgfitplot)
    {
      for (int ichan=0; ichan<nchans; ichan++)
	{
	  TH1 *dhist = datahist[ichan];
	  csm_channel_model *cm = test_hypothesis_pe->chanmodel[ichan];
	  int ntemplates = cm->histotemplate.size();
	  int nbinsx = dhist->GetNbinsX();
	  int nbinsy = dhist->GetNbinsY();
	  for (int ibinx=0;ibinx<nbinsx;ibinx++)
	    {
	      for (int ibiny=0;ibiny<nbinsy;ibiny++)
		{
		  ...
		  double b=0;
		  double d=0;
		  if (nbinsy==1) d += dhist->GetBinContent(ibinx+1);
		  else d += dhist->GetBinContent(ibinx+1,ibiny+1);
		  for (int itemplate=0;itemplate<ntemplates;itemplate++)
		    {
		      double gbc=0;
		      if (nbinsy==1) gbc = cm->histotemplate[itemplate]->GetBinContent(ibinx+1);
		      else gbc = cm->histotemplate[itemplate]->GetBinContent(ibinx+1,ibiny+1);
		      gbc *= cm->sft[itemplate];
		      if (cm->scaleflag[itemplate]) s += gbc;
		      else b += gbc;
		    }
		  if (b<0)
		    {
		      std::cout << "Negative background total in mcmc1 s/b plot" << std::endl;
		      std::cout << "Corresponding data histogram: " << dhist->GetName() << ibinx << " " << ibiny << std::endl ;
		      exit(0);
		    }
		  int ibin = dhp->GetNbinsX();  // if b=0, put contents in highest bin (overflow)
		  if (b>0)
		    {
		  ...
		  binindex.push_back(ibin);
		  shp->SetBinContent(ibin,s+shp->GetBinContent(ibin));
		  dhp->SetBinContent(ibin,d+dhp->GetBinContent(ibin));
		}
	    }
	}
    }
  ...
  if (action != bgfitplot)
    {
      const double sfbase = 1.188502227;   // the 40th root of 1000
  ...
  // do it again to get another signal resolution factor -- only need to do this if we are
  // doing a 2d cross section fit.
  ...
  if (action == xsfit2d)
    {
      ...
      for (int isf=-40;isf<41;isf++)
	{
	  double esf2 = pow(sfbase,(double) isf);
	  double esf = 1;
	  double llftest = bayesmcmcllf(test_hypothesis_pe,nnptot,npvalues,npunconstrained,npconsflag,
					nperrtot,pnpvalues,esf,esf2,prior,forcedatatobeinteger);
	  if (isf == -40 || llftest > lmx) 
	...
	{
	  stotdims2 = stotdim/2.0;
	}
    }

  // explore the space of nuisance parameters along the axes and see which ones are constrained by the data
  // skip the ones that are computed as functions of other parameters
  ...
  for (int i=0;i<nnptot;i++)
      ...
      if (!npconsflag[i])
	{
	  double llft[3],slf[3];
	  int k=0;
	  double vmin = max(nplb[i],-4.0);
	  double vmax = min(nphb[i],4.0);
	  double dv = (vmax - vmin)/16.0;
	  vmin += dv/100.0;
	  vmax -= dv/100.0;
	  ...
	  for (double v=vmin; v<vmax; v += dv)  
	      ...
	      if (v>nplb[i] && v<nphb[i])
		{
		  ...
		  double llft1 = bayesmcmcllf(test_hypothesis_pe,nnptot,npvalues,npunconstrained,npconsflag,
					      nperrtot,pnpvalues,ssf,ssf,prior,forcedatatobeinteger) - slog;
		  npvalues[i] = 0;  // undo this so we explore along the axes
		  if (k<3)
		    {
		      llft[k] = llft1;
		      slf[k] = v;
	...
	{
	  npresolutions[i] = 1.0;  // shouldn't need this as this nuisance parameter is computed as a function of other nuisance parameters
	}
    }
  ...
  for (int isample=0; isample<nmc_req; isample++)
	...
	{ for (int ibin=0;ibin<nbinslnsb;ibin++) bgfa[ibin] = 0; }
      ...
      // uniform for the signal scale factor(s).
      ...
      for (int i=0;i<nchans;i++)
	{
	      ...
	      for (int k=0;k<nbinsy;k++)
		{
		  ...
		  for (int itpl = 0; itpl < ntemplates; itpl++)
		      ...
		      if (cm->poissflag[itpl] != 0) 
			{
			  ...
			  double rte=rtmp;
			  for (int itmp=0;itmp<30;itmp++) // limit this to thirty tries
			    {
			      proposed_pnpvalues[inp] = pnpvalues[inp] + gRandom->Gaus(0,1)/stotdimp;
			  ...
			  if (cm->scaleflag[itpl] == 0)
			    {
			      ...
			      b += rtmp*cm->sft_varied[itpl];
			    }
			}
		    }
		  if (action == bgfitplot)
		    {
		      bgfa[binindex[igbin]] += b;
		      igbin++;
		    }
		}
	    }
      ...
      if (action == bgfitplot)  // fit only the background when doing a bgfitplot
	{
	  proposed_ssf = 0;
	}
      else
	{
	  do
	    {
	      double ur=0;
	      do { ur = gRandom->Uniform(1.0); } while (ur==1.0); // rethrow if we get exactly 1.0 to make it completely symmetric
	  ...
	  do
	    {
	      double ur=0;
	      do { ur = gRandom->Uniform(1.0); } while (ur==1.0); // rethrow if we get exactly 1.0 to make it completely symmetric
	      proposed_ssf2 = ssf2 + (ur - 0.5)/stotdims2;
	    }
	  while (proposed_ssf2 < 0);
	}
      else
	{
	  proposed_ssf2 = proposed_ssf;
	}

      double proposed_llf = bayesmcmcllf(test_hypothesis_pe,nnptot,proposed_npvalues,npunconstrained,npconsflag,
					 nperrtot,proposed_pnpvalues,proposed_ssf,proposed_ssf2,prior,forcedatatobeinteger);

      // since the parameter space is truncated, put in a factor for the asymmetry in the
      // step from here to another place and back again.  Do not do this for unconstrained parameters or
      // parameters that are computed as functions of other parameters
      ...
      for (int i=0;i<nnptot;i++)
	{
      ...
      if (proposed_ssf < 0.5/stotdims) proposed_llf -= log(0.5 + proposed_ssf*stotdims);
      if (action == xsfit2d && proposed_ssf2 < 0.5/stotdims2) proposed_llf -= log(0.5 + proposed_ssf2*stotdims2);
      ...
      if (ur < pratio || isample == 0)  // always accept the first one just to baseline llf 
	{
      ...
      if (isample>=nmcmc1burnin)
	{
	  if (action == bgfitplot)
	    {
	      for (int i=1;i<nbinslnsb;i++)
		{
	  ...
	  if (action==llfavg) ssflist.push_back(-2.0*llf);
	  else ssflist.push_back(ssf);

	  if (action == xsfit2d) ssf2list.push_back(ssf2);
	  if (phistflag)
	    {
	      int k=0;
	      for (int i=0;i<nnptot;i++)
  ...
  // Print out unsorted posterior samples if requested to do so

  if ( (action == xsfit1d || action == limit1d) && dumpsfp )
    {
      int nssflist = ssflist.size();
      for (int i=0; i<nssflist; i++)
	{
	  std::cout << "MCMC1D Signal Posterior Sample: " << ssflist[i] << std::endl;
	}
    }

  if ( action == llfavg && dumpsfp )
    {
      int nssflist = ssflist.size();
      for (int i=0; i<nssflist; i++)
	{
	  std::cout << "MCMC -2lnL Sample: " << ssflist[i] << std::endl;
	}
    }

  if ( (action == xsfit2d) && dumpsfp )
    {
      int nssflist = ssflist.size();
      for (int i=0; i<nssflist; i++)
	{
	  std::cout << "MCMC2D Signal Posterior Sample: " << ssflist[i] << " " << ssf2list[i] << std::endl;
	}
    }

  // only need to sort the posterior sample list if we are doing a 1D cross section fit or a 1D limit
  // find maxima to use as histogram limits
  ...
  if (action == xsfit1d || action == limit1d) 
	...
	{ 
      ...
      for (int i=0;i<nssf2list;i++)
	{ ssf2max = max(ssf2max,ssf2list[i]); }
      ssf2min = 0;
    }
  ...
  if (action == xsfit2d)
    {
      if (xs2dposterior) { local2dposterior = xs2dposterior; }  // use the input histo if we have one
      local2dposterior->Reset();
      int nssflist = ssflist.size();
      for (int i=0; i<nssflist; i++) local2dposterior->Fill(ssflist[i],ssf2list[i]);
    }

  TString ssfhtitle = "Signal Scale Factor";
  if (action == llfavg) ssfhtitle = "-2lnL";
  TH1F ssfhist("mcmcssf",ssfhtitle,100,ssf1min,ssf1max);
  TH1F *local1dposterior = &ssfhist;
  if (xs1dposterior) local1dposterior = xs1dposterior;
  local1dposterior->Reset();
  int nssflist = ssflist.size();
  ...
  // compute average L and put -2ln(avg L) answer in sflimit (so as not to add extra arguments)

  if (action == llfavg && sflimit != 0)
    {
      int nssflist = ssflist.size();
      double ssfavg = 0;
      for (int i=0;i<nssflist;i++) ssfavg += ssflist[i];
      if (nssflist>0) ssfavg /= (double) nssflist;
      double lfa = 0;
      for (int i=0;i<nssflist;i++) lfa += exp(-0.5*(ssflist[i]-ssfavg));
      if (nssflist>0) lfa /= (double) nssflist;
  ...
  if (action == xsfit1d || action == limit1d)
      ...
      do
	{
	  ...
	  double mlow = 0;
	  for (int j=1;j<ilow;j++)
	    { mlow = max(mlow,local1dposterior->GetBinContent(j)); }

	  double mhigh=0;
	  for (int j=ihigh+1;j<=nbins;j++)
...
// April 5, 2011 -- added a second signal scale factor to allow 2D cross section fits.  Always assume the
// first and second signal templates are in the same order for a 2D fit (just as in bh_xsfit_2d).
	  ...
	  for (int k=0;k<nbinsy;k++)
	    {
		  ...
		  else rtmp = cm->histotemplate_varied[itpl]->GetBinContent(j+1,k+1)*cm->sft_varied[itpl];
...
// Make systetmatically varied distributions of stacked templates.  Sample nsample times, find the median
// in each bin, and 68% and 95% intervals around the median.  Uses testhyp_pe as the model.
// This routine will create the histograms and fill in pointers to them.
  ...
  for (i=0;i<nchans;i++)
    {
	  ...
	  ichan=i;
          ...
          nbinsy = cm->histotemplate[0]->GetNbinsY();
          nbinstot = nbinsx*nbinsy;
          ahm2.reset (copy_TH1<TH1> (*cm->histotemplate[0]));
	}
    }
  ...
  for (i=0;i<(Int_t) testhyppescale->channame.size(); i++)
    {
      ...
      strcat(pdname," pseudodata");
      pdarray[i] = copy_TH1<TH1> (*testhyppescale->chanmodel[i]->histotemplate[0], pdname);
      delete pdname;
    }
  for (ipx=0;ipx<npx;ipx++)
    {
...
//  include FastTH1 code from Nils
