...
// mclimit_csm -- use a chisquared (really -2lnL) minimized over nuisance parameters
// to compute cls.  Also computes Bayesian limits and cross sections
...
public:
  ...
  // throws: std::bad_alloc
public:
  template<class Hist> Hist *copy_TH1 () const;
...
public:
  ...
  // throws: std::bad_alloc
public:
  template<class Hist> Hist *copy_TH1 (unsigned index) const;
...
public:
  ...
  // throws: std::bad_alloc
public:
  TH1 *make_TH1 (std::string name = "") const;
...
copy_TH1 (const FastTH1& that, const std::string& name)
{
  ...
  result.reset (that.make_TH1 (name));
...
#ifndef __CINT__
template<class Hist> inline Hist *TH1Input ::
copy_TH1 () const
{
  test_invariant ();
  ...
  if (slow_)
    result.reset (::copy_TH1<Hist> (*slow_));
  if (fast_)
...
#ifndef __CINT__
template<class Hist> inline Hist *TH1InputList ::
copy_TH1 (unsigned index) const
{
  ...
  if (slow_ && slow_[index])
    result.reset (::copy_TH1<Hist> (*slow_[index]));
...
#define MCLIMIT_CSM_CLSB 7
#define MCLIMIT_CSM_CLSBM2 8
#define MCLIMIT_CSM_CLSBM1 9
#define MCLIMIT_CSM_CLSBMED 10
#define MCLIMIT_CSM_CLSBP1 11
#define MCLIMIT_CSM_CLSBP2 12
...
// use to steer the choice of prior matching for asymmetric errors and errors that are fractionally large
// Truncated Gaussian with quadratic bridges (old default) or exponentiated version thereof (suggestion of W. Fisher)
// use as arguments to csm_model::setpriorfunc().  Done for each model

typedef enum {
  CSM_PRIORFUNC_QUAD,
  CSM_PRIORFUNC_EXPONENTIAL
} CSM_PRIORFUNC;
...
class csm_channel_model
{
 public:
   ...
   void setpriorfunc(CSM_PRIORFUNC);  // either CSM_PRIORFUNC_QUAD (the default), or CSM_PRIORFUNC_EXPONENTIAL (from W. Fisher)
...
private:
  ...
  friend class NK::McLimit::ExpMcLimit2;
  friend class NK::McLimit::ExpExpected;
};
...
class csm_model
{
 ...
 private:
  ...
  CSM_PRIORFUNC modelpriorfunc;  // whether to use Wade's prior impact function (an exponentiated quadratic bridge)
                                 // or the old Quadratic bridge
...
class mclimit_csm
{
 public:
   ...
   // rate limit calculators -- based on CLs
   ...
   // rate limit calculators -- based on CLsb -- to be used to compute Power-Constrained limits (PCL)
   // -- but you have to apply your own power constraint (usually at the -1 sigma expected rate limit)
   Double_t s95sb();      // scale factor on signal which is excluded at exactly 95% CL
   Double_t s95m2sb();   // variation around the median expected s95 in the bg hypothesis, -2 sigma
   Double_t s95m1sb();   // variation around the median expected s95 in the bg hypothesis, -1 sigma
   Double_t s95medsb();  // median expected s95 in the background hypothesis
   Double_t s95p1sb();   // variation around the median expected s95 in the bg hypothesis, +1 sigma
   Double_t s95p2sb();   // variation around the median expected s95 in the bg hypothesis, +2 sigma
...
// the number of burnin links in the chain.  
   ...
   // interval in the chain after which we re-evauluate the accept rate and adjust the scale factor
   // on the proposal fucntion.  Should be large, and the adjustment of the scale factor should be small.

   void set_mcmc1_accept_interval(int accept_interval=5000) {nmcmc1_accept_interval = accept_interval;};
   int get_mcmc1_accept_interval() { return nmcmc1_accept_interval;};

   // low edge of the acceptance fraction below which we rescale the proposal function

   void set_mcmc1_accept_low(double accept_low=0.3) {rmcmc1_accept_low = accept_low;};
   double get_mcmc1_accept_low() { return rmcmc1_accept_low;};

   // upper edge of the acceptance fraction above which we rescale the proposal function

   void set_mcmc1_accept_high(double accept_high=0.5) {rmcmc1_accept_high = accept_high;};
   double get_mcmc1_accept_high() { return rmcmc1_accept_high;};

   // scale factor on the proposal function for the adaptive step.  Make it gentle

   void set_mcmc1_accept_dimscale(double accept_dimscale=1.2) {rmcmc1_accept_dimscale = accept_dimscale;};
   double get_mcmc1_accept_dimscale() { return rmcmc1_accept_dimscale;};

   void disable_poisson_posteriorsinrootfile_mcmc1() {bmcmc1_poiss_in_rootfile = false;};
   void enable_poisson_posteriorsinrootfile_mcmc1() {bmcmc1_poiss_in_rootfile = true;};

// like bh_xsfit, but this one works with the Markov Chain calculation
// reports the best fit cross section value, 68% up and down uncertainties, and an option to put posteriors
// for nuisance parameters and the cross section into a rootfile, and a text dump for the signal Markov Chain
// step values.  The optional 1D Posterior histograms are there for the use to choose a binning and to manipulate later.
...
// of the Markov Chain steps for the signals
   ...
   // cross section fits.)
   // The optional 2D histogram here is again just for binning purposes.  It is filled upon return with
   // the last pseudoexperiment's posterior

   void xsfit_2d_mcmc1_expect(int npx, TH2F *xs2dposterior=0);
...
// prior is flat or correlated -- See Joel's CDF 7587, sec. 6
    ...
    // average the likelihood function over nuisance parameters and report -2lnLavg

    double minustwolnLavg_mcmc1(TString histoutfile="", PRIOR prior=flat);

    // run a single pseudoexperiment through the minustwolnLavg procedure.  Indexed by pseudoexperiment so you can go
    // back to the same pseudoexperiment.  Warning -- this routine reseeds gRandom to a function of ipx.  This is meant
    // so you can have a single pseudoexperiment that you fit again and again to different testhyyp_pe's

    double minustwolnLavg_mcmc1_pseudoexperiment(int ipx=0, TString histoutfile="", PRIOR prior=flat);

    // same thing, but use MINUIT to compute chisquared.  Warning -- this routine reseeds gRandom to a function of ipx

    double nullpe_testpe_pseudoexperiment(int ipx=0);

    // fill the given histograms with bin contents of a channel-summed distributions of log10 s/b.
    // use the Markov Chain to do a background fit, and fill entries in the bgfit profile.  The
    // uncertainties in the background fit are the RMS's of the background sum posteriors.
    // The three histograms will be reset upon calling this routine.  The three histograms should have the same binning.
...
// correlated signal prior -- used by bayeslimit_mcmc1
...
// scale factor are used to adjust predictions.  The first scale factor is applied to the first
// signal template; the second scale factor is applied to all other templates.  For 1D fits, set
// the two scale factors equal to each other.

   double bayesmcmcllf(csm_model *model, int nnptot, double *npvalues, bool *npunconstrained, bool *npconsflag,
                        int nperrtot, double *pnpvalues, double ssf, double ssf2, PRIOR prior, bool forcedatatobeinteger);

// debugging tools -- get the data histogram and dump signal, background, and data
...
// This routine will create the histograms and fill in pointers to them.
 ...
 private:
  ...
  int nmcmc1burnin;
  int nmcmc1_accept_interval;
  double rmcmc1_accept_low;
  double rmcmc1_accept_high;
  double rmcmc1_accept_dimscale;

  bool bmcmc1_poiss_in_rootfile;
};
